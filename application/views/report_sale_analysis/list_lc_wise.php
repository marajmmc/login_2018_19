    ...
    $(document).ready(function ()
    {
        ...
        var cellsrenderer = function(row, column, value, defaultHtml, columnSettings, record)
        {
            ...
            if(column=='details_button')
            {
                if(record.id)
                {
                    element.html('<div><button class="btn btn-primary pop_up" data-action-link="<?php echo site_url($CI->common_view_location.'/index/details'); ?>/'+record.id+'">View Details</button></div>');
                }
                else
                {
                    element.html('');
        ...
        $("#system_jqx_container").jqxGrid(
            {
                ...
                columns:
                    [
                        { text: '<?php echo $CI->lang->line('LABEL_CROP_NAME'); ?>', dataField: 'crop_name',pinned:true,width:'100',cellsrenderer: cellsrenderer,hidden: <?php echo $system_preference_items['crop_name']?0:1;?>},
                        ...
                        { text: '<?php echo $CI->lang->line('LABEL_PACK_SIZE'); ?>', dataField: 'pack_size',pinned:true,width:'100',cellsrenderer: cellsrenderer,hidden: <?php echo $system_preference_items['pack_size']?0:1;?>},
                        { text: '<?php echo $CI->lang->line('LABEL_BARCODE'); ?>', dataField: 'barcode',pinned:true,width:'100',cellsrenderer: cellsrenderer,hidden: <?php echo $system_preference_items['barcode']?0:1;?>},
                        { text: '<?php echo $CI->lang->line('LABEL_LC_NUMBER'); ?>', dataField: 'lc_number',pinned:true, width:110, hidden: <?php echo $system_preference_items['lc_number']?0:1;?>},
