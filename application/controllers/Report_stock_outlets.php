...
class Report_stock_outlets extends Root_Controller
{
    ...
    public function index($action="search",$id=0)
    {
        ...
        elseif($action=="set_preference_stock_current_outlets")
        {
            $this->system_set_preference_stock_current_outlets();
        }
        ...
        elseif($action=="get_items_stock_current_outlets")
        {
            $this->system_get_items_stock_current_outlets();
        }
        else
        {
            $this->system_search();
    ...
    private function system_list()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            elseif($reports['report_name']=='stock_current_outlets')
            {
                $data['title']='Showroom wise Stock Report';
                $data['system_preference_items']= $this->get_preference_stock_current_outlets();
                //get outlets
                ...
                {
                    $this->db->where('zones.division_id',$reports['division_id']);
                    if($reports['zone_id']>0)
                    {
                        $this->db->where('zones.id',$reports['zone_id']);
                        if($reports['territory_id']>0)
                        {
                            $this->db->where('territories.id',$reports['territory_id']);
                            if($reports['district_id']>0)
                            {
                                $this->db->where('districts.id',$reports['district_id']);
                                if($reports['outlet_id']>0)
                                {
                                    $this->db->where('cus_info.customer_id',$reports['outlet_id']);
                                }
                            }
                        }
                    }
                }
                $this->db->order_by('divisions.ordering','ASC');
                $this->db->order_by('zones.ordering','ASC');
                $this->db->order_by('territories.ordering','ASC');
                $this->db->order_by('cus_info.ordering','ASC');
                $data['areas']=$this->db->get()->result_array();
                $ajax['system_content'][]=array("id"=>"#system_report_container","html"=>$this->load->view($this->controller_url."/list_stock_current_outlets",$data,true));
            }
            else
            {
    ...
    private function get_preference_headers_stock_current_outlets()
    {
        $data['crop_name']= 1;
        $data['crop_type_name']= 1;
        $data['variety_name']= 1;
    ...
    private function get_preference_stock_current_outlets()
    {
        $user = User_helper::get_user();
        $result=Query_helper::get_info($this->config->item('table_system_user_preference'),'*',array('user_id ='.$user->user_id,'controller ="' .$this->controller_url.'"','method ="search_stock_current_outlets"'),1);
        $data=$this->get_preference_headers_stock_current_outlets();
        if($result)
        {
            if($result['preferences']!=null)
            {
    ...
    private function system_get_items_stock_current_outlets()
    {
        $items=array();
        $division_id=$this->input->post('division_id');
        $zone_id=$this->input->post('zone_id');
        ...
        {
            $outlet_ids[$result['value']]=$result['value'];
        }
        ...
        $this->db->from($this->config->item('table_pos_stock_summary_variety').' stock_summary_variety');
        $this->db->select('stock_summary_variety.variety_id,stock_summary_variety.pack_size_id,stock_summary_variety.current_stock');
        ...
        $this->db->where_in('stock_summary_variety.variety_id',$variety_ids);
        $this->db->where_in('stock_summary_variety.outlet_id',$outlet_ids);
        //$this->db->group_by(array($location_type));
        //$this->db->group_by('stock_summary_variety.variety_id');
        //$this->db->group_by('stock_summary_variety.pack_size_id');
        $results=$this->db->get()->result_array();
        ...
        foreach($results as $result)
        {
            $stocks[$result['variety_id']][$result['pack_size_id']][$result['outlet_id']]=$result['current_stock'];
        }
        $prev_crop_name='';
        $prev_type_name='';
        $first_row=true;
    ...
    private function system_set_preference_stock_current_outlets()
    {
        if(isset($this->permissions['action6']) && ($this->permissions['action6']==1))
        {
            $data['system_preference_items']= $this->get_preference_stock_current_outlets();
            $data['preference_method_name']='search_stock_current_outlets';
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view("preference_add_edit",$data,true));
            $ajax['system_page_url']=site_url($this->controller_url.'/index/set_preference');
            $this->json_return($ajax);
